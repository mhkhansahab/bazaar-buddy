...
function transformProduct(apiProduct: ApiProduct): TransformedProduct {
  ...
  if (hasDiscount) {
    ...
    originalPrice = Math.round(apiProduct.price * (100 / (100 - discount)) * 100) / 100;
  }
      ...
      try {
        ...
        const categoriesWithProducts = await Promise.all(
          apiCategories.map(async (category) => {
            try {
              const productsResponse = await fetch(`/api/products?category=${encodeURIComponent(category.name)}&active=true`);
              if (!productsResponse.ok) {
                console.warn(`Failed to fetch products for category ${category.name}`);
                return null;
              }
        ...
        const validCategories = categoriesWithProducts.filter((cat): cat is CategoryData => cat !== null);
